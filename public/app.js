...
// ── Mapa DH → OBs completas (cache) ──────────────────────────────────────────
let obMap = {}  // { "2026NP000152": [ {numeroOB, tipoOB, valor, dataSaque, ...} ] }
...
// ── Tipo de documento de dedução a partir do codSit SIAFI ────────────────────
function tipoDeducao(codSit) {
  if (!codSit) return codSit
  const c = String(codSit).toUpperCase()
  if (c.startsWith("DDF")) return "DARF"
  if (c.startsWith("DGD")) return "GPS"
  if (c.startsWith("DAR")) return "DAR"
  if (c.startsWith("TRF")) return "TRF"
  return codSit
}

// ── Consolidação ──────────────────────────────────────────────────────────────
async function consolidarEExibir() {
  const [dhRows, empRows, obRows] = await Promise.all([
    ipcRenderer.invoke("getDHData"),
    ipcRenderer.invoke("getPlanilhaData"),
    ipcRenderer.invoke("getOBs"),
  ])

  const planMap = {}
  for (const e of empRows) planMap[normalizarEmpenho(e.empenho)] = e

  // Montar mapa DH → lista de objetos OB completos
  obMap = {}
  for (const ob of obRows) {
    if (!ob.documentoOrigem) continue
    if (!obMap[ob.documentoOrigem]) obMap[ob.documentoOrigem] = []
    obMap[ob.documentoOrigem].push(ob)
  }

  const planVazio = {
    cnpj: "", fornecedor: "", descricao: "", rpl: "", fonte: "", ptres: "",
    naturezaDespesa: "", descNatureza: "", subitem: "", descSubitem: "",
    pi: "", grupoDespesa: "", descGrupo: "", vinculacao: "", categDespesa: ""
  }
...
function renderizarTabela() {
  ...
  for (const numeroDH of dhKeys) {
    ...
    // Estado do DH: usa statusPgto gravado no banco (vem do XML SIAFI).
    // OBs vinculadas confirmam pagamento independentemente do XML.
    const isCancelado = emps.some(r => r.statusPgto === "Cancelado")
    const isRealizado = emps.some(r => r.statusPgto === "Realizado") || (obMap[numeroDH] || []).length > 0
    let st = isCancelado ? "Cancelado" : isRealizado ? "Realizado" : "Pendente"
    const stCls = st === "Cancelado" ? "st-cancelado"
      : st === "Pendente" ? "st-ded"
      : "st-pago"
    ...
    const obsDoGrupo = obMap[numeroDH] || []

    html += `<tr class="dh-header">
      <td class="col-ck"></td>
      <td class="dh-num"><strong>${numeroDH}</strong> ${badgeNovo}</td>
      <td class="dh-processo">${first.processo || ""}</td>
      <td>${first.fornecedor || ""}</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
      <td class="valor"><strong>${fmt(totalBruto)}</strong></td>
      <td>${pfCell}</td>
      <td></td>
      <td>${transfCell}</td>
    ...
    // Sub-linhas de empenho — valor líquido (dados de pagamento = bruto - deduções)
    for (const r of emps) {
      const catEmp = r.categDespesa || categoriaPorGrupo(r.grupoDespesa)
      html += `<tr class="emp-sub">
        <td></td>
        <td class="emp-label">Pgto. Liq.</td>
        <td></td>
    ...
    // ── Linhas de OB ────────────────────────────────────────────────────
    const empUnico = emps.length === 1 ? (first.empenho || "") : ""
    for (const ob of obsDoGrupo) {
      const tipoLabel = ob.tipoOB === "11" ? "OB Principal"
        : ob.tipoOB === "12" ? "OB Impostos"
        : ob.tipoOB === "03" ? "OB PIX/Adto"
        : ob.tipoOB ? `OB Tipo ${ob.tipoOB}` : "OB"
      html += `<tr class="ob-sub">
        <td></td>
        <td class="ob-tipo">${tipoLabel}</td>
        <td class="ob-data">${fmtData(ob.dataSaque || ob.dataEmissao)}</td>
        <td class="emp-ne">${empUnico}</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td class="valor ob-val">${fmt(ob.valor)}</td>
        <td></td>
        <td><span class="doc-tag doc-ob">${ob.numeroOB}</span></td>
        <td></td>
        <td class="st-pago">Pago</td>
      </tr>`
    }

    // ── Linhas de dedução (DARF / DAR / GPS) ────────────────────────────
    for (const r of emps) {
      const deds = r._deds || []
      for (let di = 0; di < deds.length; di++) {
        ...
        const tipo     = d.codSit || tipoDeducao(d.codSit)
        const catDed   = r.categDespesa || categoriaPorGrupo(r.grupoDespesa)
        const dedData  = `data-chave="${r.chave}" data-idx="${di}" data-vlr="${d.vlr}" data-fonte="${r.fonte || ""}" data-ptres="${r.ptres || ""}" data-vinc="${r.vinculacao || ""}" data-rpl="${r.rpl || "2"}" data-grupo="${r.grupoDespesa || ""}" data-categ="${r.categDespesa || ""}" data-empenho="${r.empenho || ""}" data-numdh="${numeroDH}" data-codsit="${d.codSit}"`
        const dedTransf = d.transfDoc || ""
        const dedTransfCell = dedTransf
          ? `<span class="doc-tag doc-transf">${dedTransf} <span class="doc-remove" onclick="desvincularDedTransf('${r.chave}',${di})" title="Remover">&times;</span></span>`
          : ""
        const temTransf = !!dedTransf
        const showCk = pend && !temTransf && (d.codSit === "DDF025" || d.codSit === "DDF021")

        html += `<tr class="ded-sub">
          <td class="col-ck">${showCk ? `<input type="checkbox" class="check-ded" ${dedData} onchange="atualizarPainelDed()" />` : ""}</td>
          <td class="ded-tipo">${tipo}</td>
          <td class="ded-data">${pend ? '<span style="color:#94a3b8">pendente</span>' : fmtData(d.dtPgto)}</td>
    ...
    html += `<tr class="dh-sep"><td colspan="13"></td></tr>`
  }
...
// ── Importar OB por Planilha Excel ───────────────────────────────────────────
document.getElementById("inputOBPlanilha").addEventListener("change", async (e) => {
  ...
  try {
    ...
    const result = await ipcRenderer.invoke("importarOBPlanilha", Array.from(new Uint8Array(buffer)))
    const partes = [`${result.inseridos} OB(s) importada(s)`]
    if (result.ignorados) {
      const ig = result.ignorados
      if (ig.darf > 0)      partes.push(`${ig.darf} DARF ignorado(s)`)
      if (ig.semOB > 0)     partes.push(`${ig.semOB} sem OB ignorado(s)`)
      if (ig.canceladas > 0) partes.push(`${ig.canceladas} cancelada(s)`)
    }
    mostrarStatus(partes.join(" | "), "ok")
    await consolidarEExibir()
  } catch (err) {
    mostrarStatus("Erro ao importar planilha de OBs: " + err.message, "error")
  }
})

// ── Importar OB (Ordem Bancária) ─────────────────────────────────────────────
document.getElementById("btnImportOB").addEventListener("click", async () => {
  mostrarStatus("Aguardando seleção de PDF(s)...", "info")
  try {
    const result = await ipcRenderer.invoke("importarOBPDF")
    if (result.inseridos === 0 && result.erros.length === 0) {
      mostrarStatus("Nenhum arquivo selecionado.", "info")
      return
    }
    const partes = []
    if (result.inseridos > 0) partes.push(`${result.inseridos} OB(s) importada(s)`)
    if (result.erros.length > 0) partes.push(`${result.erros.length} erro(s): ${result.erros.join("; ")}`)
    mostrarStatus(partes.join(" | "), result.erros.length > 0 ? "error" : "ok")
    await consolidarEExibir()
  } catch (err) {
    mostrarStatus("Erro ao importar OB: " + err.message, "error")
  }
})
...
btnFecharPF.addEventListener("click", () => modalPF.classList.add("hidden"))
modalPF.addEventListener("click", (e) => { if (e.target === modalPF) modalPF.classList.add("hidden") })
