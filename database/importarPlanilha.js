...
function importarPlanilha(caminho) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Inserir: empenhos novos com INSERT OR IGNORE, existentes com UPDATE
      // seletivo (só preenche campos vazios)
      db.serialize(() => {
        ...
        for (const d of unicos) {
          // Tentar inserir (ignora se já existe)
          stmtInsert.run([
          ...
          // Atualizar campos vazios do registro existente
          stmtUpdate.run([
...
function parsarFormatoBanco(header, dados, resultado) {
  ...
  // Detectar CNPJ: pode ser "CNPJ" separado ou "CNPJ   FORNECEDOR" mesclado
  ...
  // Se "CNPJ   FORNECEDOR" está na mesma célula, CNPJ=col 0 e Fornecedor=col 1 (null header)
  if (cnpjCol >= 0 && fornCol < 0) {
    fornCol = cnpjCol + 1
  }

  // Natureza Despesa: pode ter sub-colunas (código | descrição)
  const ndCol = header.findIndex(h => h && h.includes("NATUREZA") && h.includes("DESPESA"))
  const ndDescCol = ndCol >= 0 ? ndCol + 1 : -1

  // Subitem: pode ter sub-colunas (código | descrição)
  const subCol = header.findIndex(h => h && h.includes("SUBITEM"))
  const subDescCol = subCol >= 0 ? subCol + 1 : -1

  // Grupo Despesa: pode ter sub-colunas (número | descrição)
...
function parsarFormatoDiario(header, dados, resultado) {
  ...
  const colMap = {
    neCol:     col("NE CCOR"),                                          // col 1: código NE completo
    fonteSOF:  header.findIndex(h => h && h.includes("FONTE")),         // col 2: fonte abreviada
    cnpj:      header.findIndex(h => h && h.includes("FAVORECIDO")),    // col 3: CNPJ/CPF
    ptres:     col("PTRES"),                                            // col 5
    pi:        col("PI"),                                               // col 6
    nd:        header.findIndex(h => h && h.includes("NATUREZA DESPESA")), // col 8
    descricao: header.findIndex(h => h && (h.includes("DESCRIÇÃO") || h.includes("DESCRI"))), // col 9
    contaCtb:  header.findIndex(h => h && (h.includes("CONTA CONTÁBIL") || h.includes("CONTA CONTABIL"))),
  }

  // Coluna do fornecedor: é a coluna após o CNPJ (tem header null)
  const fornecedorCol = colMap.cnpj >= 0 ? colMap.cnpj + 1 : -1

  for (const linha of dados) {
    const g = (idx) => idx >= 0 ? String(linha[idx] || "").trim() : ""

    // Extrair NE do código completo (ex: 135014132032026NE000001 → 2026NE000001)
    const rawNE = g(colMap.neCol)
    const mEmp = rawNE.match(/(\d{4}NE\d+)/i)
    if (!mEmp) continue

    // Ignorar linhas de anulação (conta 522920104 ou valor negativo)
    ...
    // Natureza Despesa: derivar grupo (primeiro dígito: 3=Corrente, 4=Capital)
    ...
    // Fonte SOF: expandir código abreviado
    let fonte = g(colMap.fonteSOF)
    if (fonte && /^\d{4}$/.test(fonte)) {
      fonte = fonte + "000000"  // ex: 1050 → 1050000000, 1000 → 1000000000
    }
    ...
    resultado.push({
      ...
      vinculacao:   "",        // não disponível neste formato
    })
  }
}
